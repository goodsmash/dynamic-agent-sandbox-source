...
const AGENT_BADGE_COLORS: Record<string, string> = {
  ...
  nanoclaw:  "bg-cyan-400/10 text-cyan-400 border-cyan-400/20",
  nemoclaw:  "bg-purple-400/10 text-purple-400 border-purple-400/20",
  swarmclaw: "bg-yellow-400/10 text-yellow-400 border-yellow-400/20",
};
...
export default function SessionView() {
  ...
  // Parse query string for swarm auto-launch
  const initialCommand = useMemo(() => {
    const search = window.location.search;
    if (!search) return undefined;
    const sp = new URLSearchParams(search);
    const swarm = sp.get("swarm");
    const goal  = sp.get("goal");
    const autopilot = sp.get("autopilot");
    if (swarm && goal) return `swarm launch ${swarm} ${goal}`;
    if (autopilot)     return `autopilot ${autopilot}`;
    return undefined;
  }, []);
  ...
  const agentType = (session as any).agentType ?? "openclaw";
  const agentLabel = AGENT_LABELS[agentType] ?? agentType;
  const badgeClass = AGENT_BADGE_COLORS[agentType] ?? AGENT_BADGE_COLORS.openclaw;
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded border font-mono uppercase tracking-wider ${badgeClass}`}>
              {agentLabel}
            </span>
          </div>
          <Button 
