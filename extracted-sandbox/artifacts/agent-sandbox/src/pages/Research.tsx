...
import {
  ...
  FlaskConical, Zap, Clock, Award, BarChart3, Sparkles,
} from "lucide-react";
    ...
    setRunState((prev) => {
      ...
      switch (event.type) {
        ...
        case "experiment_done":
          ...
          break;
        case "skill_learned":
          updated.skillsLearned = [
            ...updated.skillsLearned,
            {
              id: event.skillId ?? String(Date.now()),
              name: event.skillName ?? "Unnamed Skill",
              description: event.skillDescription ?? "",
              score: event.score ?? 0,
              iteration: event.iteration ?? 0,
            },
          ];
          updated.logs = [...updated.logs, `✨ Skill learned: ${event.skillName}`];
          break;
        case "run_complete":
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {runState && (
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                ...
                {/* Skills Learned Panel */}
                <AnimatePresence>
                  {runState.skillsLearned.length > 0 && (
                    <motion.div
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="mt-4 p-4 border border-purple-500/30 rounded-lg bg-purple-500/5"
                    >
                      <div className="text-[10px] font-mono text-purple-300 uppercase mb-3 flex items-center gap-2">
                        <Sparkles className="w-3 h-3" /> Skills Learned ({runState.skillsLearned.length})
                      </div>
                      <div className="space-y-2">
                        {runState.skillsLearned.map((skill) => (
                          <div key={skill.id} className="flex items-start gap-2">
                            <div className="w-1 h-1 rounded-full bg-purple-400 mt-1.5 shrink-0" />
                            <div className="min-w-0">
                              <div className="text-xs font-medium text-purple-200">{skill.name}</div>
                              <div className="text-[10px] text-muted-foreground leading-relaxed">{skill.description}</div>
                              <div className="text-[9px] font-mono text-purple-400/60 mt-0.5">iter {skill.iteration} · {(skill.score * 100).toFixed(0)}%</div>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div className="mt-3 text-[9px] font-mono text-muted-foreground/50">
                        These skills are persisted to DB and injected into all future agent runs.
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
