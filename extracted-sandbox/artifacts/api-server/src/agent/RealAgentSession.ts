...
export class RealAgentSession {
  ...
  async routeCommand(ws: WebSocket, raw: string): Promise<void> {
    ...
    // skills — list all learned skills from DB
    if (lower === "skills" || lower === "skill list") {
      await this.sendSkills(ws); writePrompt(ws); return;
    }

    // skill learn <name>: <description> — store a skill manually
    if (lower.startsWith("skill learn ")) {
      const rest = cmd.slice("skill learn ".length);
      const ci = rest.indexOf(":");
      if (ci > 0) {
        const name = rest.slice(0, ci).trim();
        const desc = rest.slice(ci + 1).trim();
        const id = await saveSkill({ name, description: desc, implementation: desc, agentType: "openclaw", sessionId: this.sessionId, source: "manual", score: 0.7 });
        send(ws, { type: "system", data: `\x1b[32m✓ Skill "${name}" saved (id: ${id}). All agents can now use it.\x1b[0m` });
      } else {
        send(ws, { type: "error", data: "Usage: skill learn <name>: <description and implementation>" });
      }
      writePrompt(ws); return;
    }

    // skill delete <id>
    if (lower.startsWith("skill delete ")) {
      const id = cmd.slice("skill delete ".length).trim();
      const { deleteSkill } = await import("../lib/skillsManager.js");
      await deleteSkill(id);
      send(ws, { type: "system", data: `\x1b[32m✓ Skill ${id} deleted.\x1b[0m` });
      writePrompt(ws); return;
    }
  ...
  private async runChat(ws: WebSocket, userMessage: string): Promise<void> {
    ...
    const dbSkills = await loadSkillsForAgent("openclaw", 8);
  ...
  async runIsolate(ws: WebSocket, task: string): Promise<{ output: string; exitCode: number; timeMs: number }> {
    ...
    const skills = this.memory.get("SKILLS") || "";
    const memoryContent = this.memory.get("MEMORY") || "";
    const dbSkills = await loadSkillsForAgent("openclaw", 6);
    const dbSkillsText = formatSkillsForPrompt(dbSkills);
...
\x1b[33mSelf-Learning Skills:\x1b[0m
  \x1b[36mskills\x1b[0m                  List all skills learned from research + experience
  \x1b[36mskill learn <name>: <desc>\x1b[0m  Save a reusable skill to the persistent store
  \x1b[36mskill delete <id>\x1b[0m       Remove a skill by ID
...
\x1b[33mModel Control:\x1b[0m
  ...
  \x1b[36mmodels\x1b[0m                  Browse all 23 providers and their models
...
\x1b[90mSkills are automatically extracted from AutoResearch experiments (score ≥ 65%).\x1b[0m
\x1b[90mConfigure AI provider API keys → Settings (/settings)\x1b[0m`,
    });
  }

  private async sendSkills(ws: WebSocket): Promise<void> {
    const skills = await loadSkillsForAgent("any", 50);
    if (skills.length === 0) {
      send(ws, { type: "output", data: "\x1b[90mNo skills yet. Skills are learned from AutoResearch experiments (score ≥ 65%) or via 'skill learn <name>: <desc>'.\x1b[0m" });
      return;
    }
    const lines = [
      `\x1b[1m\x1b[32mLearned Skills (${skills.length})\x1b[0m`,
      `\x1b[90mSkills are injected into every task to improve agent performance.\x1b[0m`,
      "",
      ...skills.map((s) =>
        `\x1b[36m[${s.id}]\x1b[0m \x1b[1m${s.name}\x1b[0m \x1b[90m(${s.category} · ${(s.score * 100).toFixed(0)}% · ${s.source})\x1b[0m\n  ${s.description}`
      ),
      "",
      `\x1b[90mTo delete: skill delete <id>  |  To add: skill learn <name>: <description>\x1b[0m`,
    ];
  ...
  private sendModels(ws: WebSocket): void {
    send(ws, {
      type: "output",
      data: `\x1b[1m\x1b[32mOpenClaw — 23 AI Providers · 200+ Models\x1b[0m
Configure API keys at /settings — then use: \x1b[36mmodel <name>\x1b[0m

\x1b[1m\x1b[32m── ZERO-CONFIG (no API key needed) ────────────────────\x1b[0m

\x1b[33mOpenAI\x1b[0m \x1b[90m(via Replit AI proxy)\x1b[0m
  gpt-5.2  gpt-5-mini  o3  o4-mini  gpt-4o

\x1b[33mOllama\x1b[0m \x1b[90m(local, 100% free, install ollama.com)\x1b[0m
  ollama/llama3.3  ollama/qwen2.5:7b  ollama/deepseek-r1:8b  ollama/phi4

\x1b[1m\x1b[32m── ULTRA-FAST INFERENCE ────────────────────────────────\x1b[0m

\x1b[33mGroq\x1b[0m \x1b[90m(700+ tok/s · free tier)\x1b[0m
  llama-3.3-70b-versatile   llama-3.1-8b-instant   qwen-qwq-32b
  deepseek-r1-distill-llama-70b   mixtral-8x7b-32768

\x1b[33mCerebras\x1b[0m \x1b[90m(2000+ tok/s wafer-scale · free tier)\x1b[0m
  llama3.3-70b   llama3.1-70b   llama3.1-8b

\x1b[33mSambaNova\x1b[0m \x1b[90m(world's fastest 405B · free tier)\x1b[0m
  Meta-Llama-3.3-70B-Instruct   Meta-Llama-3.1-405B-Instruct   DeepSeek-R1

\x1b[33mFireworks AI\x1b[0m \x1b[90m(cheap, fast open models)\x1b[0m
  accounts/fireworks/models/llama-v3p3-70b-instruct
  accounts/fireworks/models/deepseek-r1
  accounts/fireworks/models/qwen2p5-72b-instruct

\x1b[1m\x1b[32m── FRONTIER MODELS ─────────────────────────────────────\x1b[0m

\x1b[33mAnthropic\x1b[0m
  claude-opus-4-5   claude-3-7-sonnet-20250219   claude-3-5-haiku-20241022

\x1b[33mxAI (Grok)\x1b[0m
  grok-3   grok-3-mini   grok-3-mini-fast   grok-2-vision-1212
...
\x1b[33mDeepSeek\x1b[0m \x1b[90m($0.27/1M tokens — cheapest frontier)\x1b[0m
  deepseek-chat (V3)   deepseek-reasoner (R1)

\x1b[1m\x1b[32m── MULTI-MODEL HUBS ────────────────────────────────────\x1b[0m

\x1b[33mOpenRouter\x1b[0m \x1b[90m(300+ models, many free)\x1b[0m
...
\x1b[33mTogether AI\x1b[0m \x1b[90m(100+ OSS models)\x1b[0m
  meta-llama/Llama-3.3-70B-Instruct-Turbo
  meta-llama/Llama-3.1-405B-Instruct-Turbo
  Qwen/Qwen2.5-72B-Instruct-Turbo   deepseek-ai/DeepSeek-R1

\x1b[33mDeepInfra\x1b[0m \x1b[90m(cheapest inference)\x1b[0m
  deepinfra/meta-llama/Meta-Llama-3.1-70B-Instruct
  deepinfra/Qwen/QwQ-32B   deepinfra/deepseek-ai/DeepSeek-R1

\x1b[33mNovita AI\x1b[0m
  novita/meta-llama/llama-3.3-70b-instruct   novita/deepseek/deepseek-v3

\x1b[33mHyperbolic\x1b[0m
  hyperbolic/meta-llama/Llama-3.3-70B-Instruct
  hyperbolic/deepseek-ai/DeepSeek-R1

\x1b[1m\x1b[32m── SPECIALIZED ─────────────────────────────────────────\x1b[0m

\x1b[33mMistral AI\x1b[0m
  mistral-large-latest   codestral-latest   open-mixtral-8x22b

\x1b[33mPerplexity\x1b[0m \x1b[90m(web search built-in)\x1b[0m
  sonar-pro   sonar   sonar-reasoning-pro

\x1b[33mCohere\x1b[0m \x1b[90m(best for RAG)\x1b[0m
  command-a-03-2025   command-r-plus   command-r

\x1b[33mAI21 Labs\x1b[0m \x1b[90m(Jamba SSM hybrid · 256K context)\x1b[0m
  jamba-1.5-large   jamba-1.5-mini

\x1b[33mMoonshot / Kimi\x1b[0m \x1b[90m(128K context, bilingual)\x1b[0m
  moonshot-v1-128k   moonshot-v1-32k   moonshot-v1-8k

\x1b[33mQwen / Alibaba\x1b[0m \x1b[90m(direct DashScope API)\x1b[0m
  qwen-max   qwen-plus   qwen-turbo   qwen-long

\x1b[33mZhipu AI (GLM)\x1b[0m \x1b[90m(bilingual Chinese/English)\x1b[0m
  glm-4-plus   glm-4-flash   glm-4-air   glm-4-long

\x1b[33mCloudflare Workers AI\x1b[0m \x1b[90m(CF deploy only)\x1b[0m
  @cf/meta/llama-4-scout-17b-16e-instruct   @cf/moonshotai/kimi-k2.5
  @cf/openai/gpt-oss-120b   @cf/qwen/qwen2.5-coder-32b-instruct

\x1b[90mConfigure all provider API keys → go to Settings (/settings)\x1b[0m`,
    });
  }
