...
export class NanoClawSession {
  ...
  private async route(ws: WebSocket, raw: string): Promise<void> {
    ...
    if (lower === "skills" || lower === "skill list") {
      ...
      if (skills.length === 0) {
        send(ws, { type: "output", data: "\x1b[90mNo skills yet. Run AutoResearch or: skill learn <name>: <desc>\x1b[0m" });
      } else {
        send(ws, { type: "output", data: `\x1b[1m\x1b[36mNanoClaw Learned Skills (${skills.length})\x1b[0m\n` + skills.map(s => `• \x1b[1m${s.name}\x1b[0m \x1b[90m(${s.category}, ${(s.score*100).toFixed(0)}%)\x1b[0m: ${s.description}`).join("\n") });
      }
      writePrompt(ws); return;
    }
    ...
    if (lower.startsWith("skill learn ")) {
      ...
      const ci = rest.indexOf(":");
      if (ci > 0) {
        const id = await saveSkill({ name: rest.slice(0, ci).trim(), description: rest.slice(ci + 1).trim(), implementation: rest.slice(ci + 1).trim(), agentType: "nanoclaw", sessionId: this.sessionId, source: "manual", score: 0.7 });
        send(ws, { type: "system", data: `\x1b[36m✓ Skill saved (${id}).\x1b[0m` });
      } else {
        send(ws, { type: "error", data: "Usage: skill learn <name>: <description>" });
      }
      writePrompt(ws); return;
    }
  ...
  private async runChat(ws: WebSocket, message: string): Promise<void> {
    ...
    const dbSkills = await loadSkillsForAgent("nanoclaw", 5);
    ...
    const systemPrompt = `You are NanoClaw, a ultra-fast AI agent. Be brief, precise, and structured.${
      this.memory.get("SKILLS") ? ` Built-in skills: ${this.memory.get("SKILLS")}.` : ""
    }${dbSkillsText ? `\n${dbSkillsText}` : ""}`;
