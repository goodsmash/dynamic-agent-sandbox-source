...
import { Terminal, Plus, Cpu, Activity, Shield, ChevronRight, BookOpen, Workflow, Settings, FlaskConical } from "lucide-react"
...
export function Sidebar() {
  ...
  const [newSessionAgentType, setNewSessionAgentType] = useState<"openclaw" | "nanoclaw" | "nemoclaw">("openclaw");
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-md bg-card border-border">
          <DialogHeader>
            ...
            <DialogDescription>
              Provision a new sandboxed AI agent with durable memory. Choose any of 11 providers and 300+ models.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {([
                  { id: "openclaw", label: "OpenClaw", desc: "Full-featured", color: "border-primary/40 bg-primary/10 text-primary" },
                  { id: "nanoclaw", label: "NanoClaw", desc: "Ultra-fast", color: "border-cyan-400/40 bg-cyan-400/10 text-cyan-400" },
                  { id: "nemoclaw", label: "NemoClaw", desc: "Deep reasoning", color: "border-purple-400/40 bg-purple-400/10 text-purple-400" },
                ] as const).map(agent => (
                  <button
                    key={agent.id}
                    type="button"
              ...
              {newSessionAgentType === "nanoclaw" && (
                <p className="text-[10px] text-cyan-400/70 font-mono">Fast · 1024 tok · 10 parallel · gpt-5-mini default</p>
              )}
              {newSessionAgentType === "nemoclaw" && (
                <p className="text-[10px] text-purple-400/70 font-mono">Reasoning · 16384 tok · 5 parallel · o4-mini default</p>
              )}
              {newSessionAgentType === "openclaw" && (
                <p className="text-[10px] text-primary/70 font-mono">Full-featured · 4096 tok · 20 parallel · gpt-5.2 default</p>
              )}
            </div>
            <div className="space-y-2">
              <label className="text-xs font-medium text-foreground uppercase tracking-wider flex items-center justify-between">
                <span>AI Model</span>
                <span className="text-[10px] text-primary/70 font-mono normal-case">11 providers · 300+ models</span>
              </label>
              <select 
