...
export function XTerminal({ sessionId, agentType = "openclaw" }: XTerminalProps) {
  ...
  useEffect(() => {
    ...
    const agentDoClass = agentType === "nanoclaw" ? "NanoClawAgent" : agentType === "nemoclaw" ? "NemoClawAgent" : "OpenClawAgent";
    const agentLabel = agentType === "nanoclaw" ? "NanoClaw" : agentType === "nemoclaw" ? "NemoClaw" : "OpenClaw";
    const agentColor = agentType === "nanoclaw" ? "\x1b[36m" : agentType === "nemoclaw" ? "\x1b[35m" : "\x1b[32m";
    ...
    if (cfUrl) {
      ...
      term.writeln(`${agentColor}✓  ${agentLabel} active\x1b[0m \x1b[90m(real AI — no simulation)\x1b[0m`);
      term.writeln("\x1b[90m   Parallel calls → Promise.all | Streaming → real tokens\x1b[0m");
