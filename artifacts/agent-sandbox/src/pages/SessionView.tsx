      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 shrink-0">
          <div className="flex items-center gap-3">
            ...
            {(session as any).agentType && (session as any).agentType !== "openclaw" && (
              <span className={`text-[10px] px-2 py-0.5 rounded border font-mono uppercase tracking-wider ${
                (session as any).agentType === "nanoclaw"
                  ? "bg-cyan-400/10 text-cyan-400 border-cyan-400/20"
                  : "bg-purple-400/10 text-purple-400 border-purple-400/20"
              }`}>
                {(session as any).agentType}
              </span>
            )}
          </div>
          <Button 
        ...
        <div className="flex-1 flex min-h-0 relative overflow-hidden">
          {/* Main Terminal Area */}
          <Terminal sessionId={session.id} agentType={(session as any).agentType ?? "openclaw"} />
