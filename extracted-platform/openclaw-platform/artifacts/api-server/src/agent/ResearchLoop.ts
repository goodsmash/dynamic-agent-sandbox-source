...
export type ResearchEvent =
  ...
  | { type: "run_complete"; runId: string; totalIterations: number; bestScore: number; bestIteration: number }
...
function buildProgramPrompt(goal: string, history: string, currentCode: string): string {
  ...
  (async () => {
    ...
    try {
      for (let iter = 1; iter <= params.maxIterations; iter++) {
        ...
        const proposalPrompt = buildProgramPrompt(params.goal, buildHistory(experiments), currentCode);
      ...
      emit({ type: "run_complete", runId, totalIterations: experiments.length, bestScore, bestIteration });
      emit({ type: "log", runId, message: `\n✅ Research complete! Best score: ${bestScore.toFixed(3)} at iteration ${bestIteration}` });
