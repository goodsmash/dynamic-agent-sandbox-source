...
export class RealAgentSession {
  ...
  onConnect(ws: WebSocket): void {
    ...
    send(ws, {
      ...
      data: [
        `\x1b[32m╔══════════════════════════════════════════════════╗\x1b[0m`,
        `\x1b[32m║\x1b[0m  \x1b[1mOpenClaw Agent\x1b[0m — Real AI Active                \x1b[32m║\x1b[0m`,
        `\x1b[32m╚══════════════════════════════════════════════════╝\x1b[0m`,
        `\x1b[90mSession: ${this.sessionId}\x1b[0m`,
        `\x1b[90mModel:    ${this.model}\x1b[0m`,
        `\x1b[90mProvider: ${providerLabel(this.model)}\x1b[0m`,
        `\x1b[90mMemory:   In-session (deploy to CF for persistent SQLite)\x1b[0m`,
        `\x1b[90mMode:     WebSocket | multi-provider AI | real Promise.all parallelism\x1b[0m`,
        ``,
        `\x1b[36mReal AI is active.\x1b[0m Type any task or \x1b[33mhelp\x1b[0m for commands.`,
      ].join("\n"),
    });
    writePrompt(ws);
  }
  ...
  async routeCommand(ws: WebSocket, raw: string): Promise<void> {
    ...
    if (lower.startsWith("ping")) {
      ...
      send(ws, { type: "output", data: `PING ${target}: 56 data bytes\n64 bytes from ${target}: time=${Math.floor(Math.random() * 8 + 1)}ms\n64 bytes from ${target}: time=${Math.floor(Math.random() * 8 + 1)}ms\n\x1b[90m(network from isolate: BLOCKED)\x1b[0m` });
      writePrompt(ws);
      return;
    }
  ...
  private async runParallel(ws: WebSocket, count: number, task: string): Promise<void> {
    ...
    send(ws, {
      type: "system",
      data: `\x1b[33m⚡ Spawning ${count} parallel isolates simultaneously...\x1b[0m\n\x1b[90m  All ${count} OpenAI calls launch at t=0 (Promise.all — true parallelism)\x1b[0m`,
    });
  ...
  private async runWorkflow(ws: WebSocket, workflowId: string): Promise<void> {
    ...
    send(ws, {
      ...
      data: [
        ...
        `\x1b[90m  Total time: ${wallMs}ms | ${workflow.steps.length} steps executed\x1b[0m`,
        `\x1b[90m  Deploy to Cloudflare for real V8 isolates and Workers AI (100+ models)\x1b[0m`,
      ].join("\n"),
    });
  }
  ...
  private sendHelp(ws: WebSocket): void {
    send(ws, {
      type: "output",
      data: `\x1b[1m\x1b[32mOpenClaw Agent Commands (v2.0.0 — REAL AI)\x1b[0m

\x1b[33mReal AI Execution:\x1b[0m
  \x1b[36mchat <message>\x1b[0m          Streaming multi-turn conversation (real GPT)
  \x1b[36m<any text>\x1b[0m              Execute task in sandboxed isolate (real AI)
  \x1b[36mparallel <N> <task>\x1b[0m     Spawn N real concurrent AI calls (Promise.all)
  \x1b[36mworkflow run <id>\x1b[0m       Multi-step agentic workflow with real AI per step

\x1b[33mMemory (session-persistent):\x1b[0m
  \x1b[36mmemory\x1b[0m                  Show all memory files
  \x1b[36mremember <K>: <V>\x1b[0m       Update a memory file

\x1b[33mWorkflows:\x1b[0m
  \x1b[36mworkflow list\x1b[0m           List all 12 workflow templates
  \x1b[36mworkflow run <id>\x1b[0m       Run a workflow (real AI per step)

\x1b[33mModel Control:\x1b[0m
  \x1b[36mmodel <name>\x1b[0m            Switch AI model (gpt-5.2, gpt-5-mini, etc.)
  \x1b[36mmodels\x1b[0m                  List Workers AI models (for Cloudflare deploy)

\x1b[33mHistory:\x1b[0m
  \x1b[36mhistory\x1b[0m                 Show conversation history
  \x1b[36mclear\x1b[0m                   Clear conversation history
...
\x1b[90mDeploy to Cloudflare for real V8 isolates, Workers AI, and Durable Objects:\x1b[0m
\x1b[90m  cd cloudflare-worker && npm run deploy\x1b[0m`,
    });
  }
...
\x1b[90mConfigure all providers → go to Settings (/settings)\x1b[0m`,
  ...
  private sendWhoami(ws: WebSocket): void {
    send(ws, {
      ...
      data: [
        ...
        `\x1b[32mRuntime:\x1b[0m   Node.js + WebSocket (deploy CF for Durable Objects)`,
        `\x1b[32mIsolates:\x1b[0m  Real parallel multi-provider AI calls (Promise.all)`,
        `\x1b[32mMemory:\x1b[0m    In-session Map (CF DO has persistent SQLite)`,
        `\x1b[32mProviders:\x1b[0m 11 supported (OpenAI, Anthropic, Groq, Together, OpenRouter, Mistral, Gemini, Cohere, Perplexity, Ollama, LM Studio)`,
        `\x1b[32mDeploy:\x1b[0m    cd cloudflare-worker && npm run deploy`,
      ].join("\n"),
    });
  }
  ...
  private sendStatus(ws: WebSocket): void {
    send(ws, {
      type: "output",
      data: `Agent Status: \x1b[32mACTIVE\x1b[0m
Session: ${this.sessionId}
Model: ${this.model}
Provider: ${providerLabel(this.model)}
Memory Keys: ${this.memory.size}
Conversation: ${this.conversationHistory.length} messages
Runtime: Node.js + multi-provider WebSocket bridge
Providers: OpenAI, Anthropic, Groq, Together, OpenRouter, Mistral, Gemini, Cohere, Perplexity, Ollama, LM Studio
Network: Open (isolate network blocked in CF deploy)`,
    });
  }
  ...
  private sendVersion(ws: WebSocket): void {
    send(ws, {
      type: "output",
      data: `OpenClaw Platform v2.0.0
  Local Mode:       Node.js + Express + WebSocket
  ...
  Active Provider:  ${providerLabel(this.model)}
  Production Mode:  Cloudflare Workers + Durable Objects + Dynamic Workers LOADER
  Workers AI:       100+ models via AI binding (production only)
  ...
  private sendEnv(ws: WebSocket): void {
    send(ws, {
      type: "output",
      data: `AGENT_RUNTIME=node-websocket-bridge
AI_MODEL=${this.model}
AI_PROVIDER=${providerLabel(this.model)}
MULTI_PROVIDER=true
SANDBOX_MODE=local-dev
NETWORK_POLICY=open (CF deploy: isolated)
